 ...
 * Trustpilot (Playwright), Google Places (API), BBB (HTTP),
 * App Store (iTunes API), Google Play (npm scraper)
 * → Notion Traffic Schools DB → Site
 *
 * KEY RULES:
 * 1. Never overwrite a value with null — if a source fails, keep previous data
 * 2. Log all failures to the Issues system for triage
 * 3. Trustpilot uses Playwright (HTTP fetch gets blocked)
 */
...
async function scrapeTrustpilot(
  ...
  try {
    await browserPage.goto(reviewUrl, { waitUntil: "domcontentloaded", timeout: 15000 });
    await browserPage.waitForTimeout(3000);

    const text = await browserPage.evaluate(() => document.body.innerText);
    const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);

    // Find rating and count: "Reviews 26,951" then "4.8"
    ...
    // Don't throw — just return null result
...
async function searchGooglePlaces(schoolName: string, website: string): Promise<GoogleResult | null> {
  ...
  try {
    const res = await fetch("https://places.googleapis.com/v1/places:searchText", {
      ...
      headers: {
        ...
        "X-Goog-Api-Key": PLACES_API_KEY,
        "X-Goog-FieldMask": "places.id,places.displayName,places.rating,places.userRatingCount,places.googleMapsUri,places.websiteUri",
      },
      body: JSON.stringify({ textQuery: `${schoolName} online traffic school` }),
    });
    if (!res.ok) return null;
    const data = (await res.json()) as any;
...
async function scrapeAppStore(appStoreUrl: string): Promise<{ rating: number | null; reviewCount: number | null }> {
  ...
  try {
    ...
    const res = await fetch(`https://itunes.apple.com/lookup?id=${appIdMatch[1]}&country=us`);
    const data = (await res.json()) as any;
...
async function scrapePlayStore(playStoreUrl: string): Promise<{ rating: number | null; reviewCount: number | null }> {
...
async function synthesiseReviews(
  ...
  if (reviewSnippets.length === 0) return { good: "", bad: "" };
  try {
...
async function writeToNotion(pageId: string, updates: Record<string, any>) {
  ...
  for (const [field, value] of Object.entries(updates)) {
    ...
    } else if (field.includes("Confidence")) {
      properties[field] = { select: { name: String(value) } };
    } else if (field.includes("Good") || field.includes("Bad") || field.includes("Place ID")) {
      properties[field] = { rich_text: [{ text: { content: String(value) } }] };
  ...
  try {
    ...
    for (const [key, val] of Object.entries(properties)) {
      try {
        await notion.pages.update({ page_id: pageId, properties: { [key]: val } as any });
      } catch {
...
async function main() {
  ...
  // Launch browser once for all Trustpilot scrapes
  ...
  for (const school of schools) {
    ...
    if (school.reviewUrl) {
      ...
      const tp = await scrapeTrustpilot(school.reviewUrl, page);
      if (tp.rating !== null && (tp.rating > 0 || (tp.reviewCount ?? 0) > 0)) {
        ...
        if (tp.snippets.length > 0) {
          ...
          if (synthesis.good) { updates["Review Highlights Good"] = synthesis.good; console.log(`    Good: ${synthesis.good}`); }
          if (synthesis.bad) { updates["Review Highlights Bad"] = synthesis.bad; console.log(`    Bad: ${synthesis.bad}`); }
        }
      } else {
        console.log("    FAILED — keeping previous score");
        logIssue(
          `Trustpilot scrape failed for ${school.name}`,
          "Trustpilot", "Warning", school.slug,
          `URL: ${school.reviewUrl}. Could not extract rating. Previous rating preserved: ${school.prevTrustpilot}`
        );
        // Do NOT write null — previous data stays intact
      }
    ...
    if (school.googlePlaceConfidence !== "Wrong match") {
      ...
      if (google && google.rating !== null) {
        ...
        console.log("    FAILED — keeping previous score");
        logIssue(
          `Google Places lookup failed for ${school.name}`,
          "Google", "Info", school.slug,
          `Place ID: ${school.googlePlaceId || "(none)"}. Previous rating preserved: ${school.prevGoogle}`
        );
      }
    } else {
      console.log("  Google Places... skipped (Wrong match)");
    }
    ...
    if (school.bbbUrl) {
      ...
      if (grade && grade !== "NR") {
        ...
        console.log("    FAILED — keeping previous grade");
        logIssue(
          `BBB scrape failed for ${school.name}`,
          "BBB", "Info", school.slug,
          `URL: ${school.bbbUrl}. Could not extract grade.`
        );
      }
    }
    ...
    if (school.appStoreUrl) {
      ...
      if (as.rating !== null) {
        ...
        console.log("    FAILED — keeping previous score");
        logIssue(
          `App Store lookup failed for ${school.name}`,
          "App Store", "Info", school.slug,
          `URL: ${school.appStoreUrl}. Previous rating preserved: ${school.prevAppStore}`
        );
      }
    }
    ...
    if (school.playStoreUrl) {
      ...
      if (ps.rating !== null) {
        ...
        console.log("    FAILED — keeping previous score");
        logIssue(
          `Play Store lookup failed for ${school.name}`,
          "Play Store", "Info", school.slug,
          `URL: ${school.playStoreUrl}. Previous rating preserved: ${school.prevPlayStore}`
        );
      }
    }

    // Metadata — only update scrape status if at least one source worked
    if (anySourceSucceeded) {
      updates["Review Scrape Status"] = "OK";
    } else if (Object.keys(updates).length === 0) {
      // All sources failed — flag but don't overwrite existing data
      updates["Review Scrape Status"] = "Failed";
      logIssue(
        `All review sources failed for ${school.name}`,
        "Trustpilot", "Critical", school.slug,
        "No data could be fetched from any source. All previous ratings preserved."
      );
    }
    updates["date:Reviews Last Scraped:start"] = TODAY;

    // Write to Notion (only non-null values)
  ...
  console.log("  Sources: Trustpilot (Playwright), Google, BBB, App Store, Play Store");
}
