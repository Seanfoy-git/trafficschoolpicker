...
19. [Admin Guide — Day-to-Day Operations](#19-admin-guide)
20. [Maintenance Checklists](#20-maintenance-checklists)
...
- 6 curated Tier 1/2 schools with multi-platform ratings
- 1,000+ DMV-scraped directory schools (CA + TX)
...
| Database | Notion API (5 databases) | Headless CMS — no admin panel to build |
...
| DNS/CDN | Cloudflare | DNS, SSL, redirect rules |
...
│  (6 curated)       (1000+ scraped)     (51 rows)            │
│                                                               │
│  School Pricing    State FAQs                                │
│  (school×state)    (70+ verified facts)                      │
└────────────┬──────────────────────────────────┬───────────────┘
...
│  6 Review Pages (ISR 24h)  │  Homepage (ISR 24h)            │
...
│                                                               │
│  1. Scrape CA/TX/FL DMV sources → Notion Directory           │
...
| One Liner | text | Tagline shown on cards |
...
DMV-scraped official school listings. Populated entirely by scrapers.

| Field | Type | Purpose |
|-------|------|---------|
| School Name | title | Official DMV name |
| License Number | text | State license ID (e.g. "E1988") |
| State | select | "California" / "Texas" etc. |
| Phone | text | Contact number |
| Address | text | Physical address |
| Website | url | Enriched via Google Places |
| Online Available | checkbox | Offers online courses |
| Source | select | "CA DMV" / "TX TDLR" / "FL DHSMV" |
| Date Scraped | date | Last scrape timestamp |

### 4.3 States DB (NOTION_STATES_DB)

One row per state (51 rows including DC). Populated manually.
...
```
State DMV websites ──→ Scraper scripts ──→ Notion Directory DB
Trustpilot/BBB/etc ──→ Review scraper  ──→ Notion Schools DB
School pricing pages ─→ Price scraper  ──→ Notion Pricing DB
Manual research ──────→ Admin edits    ──→ Notion States DB + FAQs DB

Notion databases ──→ lib/notion.ts ──→ Next.js pages (ISR 24h)
```
...
**"Online — ticket dismissal"** (CA, TX, FL, NY, etc.):
1. Hero with school count
2. Tier 1 comparison cards (full detail, ratings, pros/cons, price, CTA)
3. Tier 2 "More options" compact rows
4. State rules & requirements (eligibility, court process, DMV link)
5. FAQ accordion with JSON-LD
6. Full directory table (searchable, all DMV-licensed schools)
...
- Pricing: "Varies by state" (no hardcoded price)
...
|-----------|------|---------|
| `SchoolCard` | Server | Full school comparison card with ratings, price, pros/cons |
| `ComparisonTable` | Client | Sortable comparison table (by price, rating, time) |
| `MultiRating` | Server | Shows Trustpilot + Google + App Store + Play Store + BBB badges |
| `MultiRatingCompact` | Server | Condensed version for table rows |
| `ReviewSynthesis` | Server | "What reviewers say" good/bad block |
...
| `FaqSection` | Server | Accordion FAQ with JSON-LD schema |
...
- 18 states, 70+ verified facts (as of April 2026)
...
| Step | Script | Source | Target | Method |
|------|--------|--------|--------|--------|
| 1 | `scrape:ca` | CA DMV Salesforce site | Notion Directory | Playwright |
| 2 | `scrape:tx` | TX TDLR CSV download | Notion Directory | HTTP fetch |
| 3 | `scrape:fl` | FL DHSMV website | Notion Directory | Playwright |
| 4 | `scrape:reviews` | Trustpilot, BBB, App Store, Play Store | Notion Schools | HTTP + APIs |
| 5 | `scrape:prices` | School pricing pages | Notion Pricing | Playwright |
| 6 | `enrich:places` | Google Places API | Notion Directory | API |
| 7 | Deploy | Vercel deploy hook | Live site | HTTP POST |

### Rate limits respected:
- Notion: 350ms between writes (3 req/sec)
- Trustpilot: 2s between page loads
- Google Places: 150ms between lookups
- Playwright scraping: natural page load delays
...
|----------|--------|------|------|
| Trustpilot | HTTP + JSON-LD parse | Free | Rating, count, review snippets |
...
### Review synthesis:
- Extracts 10+ review snippets from Trustpilot
...
- Not on the school record itself (allows different prices per state)
- Price scraper reads targets from `scripts/config/price-sources.ts`

### Price display priority:
1. State-specific price from Pricing DB
2. If null → shows "Check website" (never shows $0)
3. Schools with no price sort to bottom of comparison tables
...
## 19. Admin Guide — Day-to-Day Operations
...
3. Set State Codes ("all" or "CA,TX,FL")
4. Set Affiliate URL if available
5. Trigger redeploy
...
### Quarterly (2 hours)
- [ ] Full audit of all 6 curated school prices vs actual websites
...
│   ├── [state]/
│   │   └── page.tsx            # Dynamic state pages (50 states)
│   ├── blog/
...
├── components/
│   ├── SchoolCard.tsx          # School comparison card
│   ├── ComparisonTable.tsx     # Sortable comparison table
│   ├── MultiRating.tsx         # Multi-platform rating badges
...
│   ├── FaqSection.tsx          # FAQ accordion + JSON-LD
│   ├── SchoolFAQ.tsx           # Legacy FAQ component (fallback)
...
├── lib/
│   ├── notion.ts               # Notion API client (all DB queries)
│   ├── notion-faqs.ts          # FAQ fetching from Notion
│   ├── types.ts                # All TypeScript types
│   ├── seo-config.ts           # Centralised SEO metadata
│   ├── state-utils.ts          # State slug/code/name mapping
│   ├── state-faqs.ts           # Static fallback FAQs
...
├── scripts/
│   ├── scrape-ca-dmv.ts        # CA DMV directory scraper (Playwright)
│   ├── scrape-tx-tdlr.ts       # TX TDLR CSV scraper
│   ├── scrape-fl-dhsmv.ts      # FL DHSMV scraper (Playwright)
│   ├── scrape-reviews.ts       # Multi-platform review scraper
│   ├── scrape-prices.ts        # School pricing scraper
...
│   └── llms-full.txt           # Auto-generated state facts
├── .github/
│   └── workflows/
